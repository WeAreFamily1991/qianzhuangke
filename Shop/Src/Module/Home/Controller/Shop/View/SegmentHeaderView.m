...
#define NORMAL_FONT [UIFont systemFontOfSize:14]
#define NORMAL_COLOR RGBHex(0X888888)
#define SELECTED_COLOR REDCOLOR
...
//    SegmentHeaderViewCollectionViewCell *cell = [self getCell:0];
//    CGFloat firstCellWidth = [self getWidthWithContent:self.titleArray[0]];
    ...
    [self.moveLine mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(SCREEN_WIDTH/12);
    }];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return CGSizeMake(SCREEN_WIDTH/3, SegmentHeaderViewHeight - 1);
}
...
- (UIView *)moveLine {
    if (!_moveLine) {
        ...
        _moveLine.backgroundColor = REDCOLOR;
    }
    return _moveLine;
}
...
- (UIView *)separator {
    if (!_separator) {
        ...
        _separator.backgroundColor = BACKGROUNDCOLOR;
    }
    return _separator;
}
