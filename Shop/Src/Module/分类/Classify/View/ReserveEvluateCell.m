...
//        [_contentLab setChangeHeight:commContentSzie.height];
    ...
    if (picList.count > 0) {
        ...
        if (picList.count <=3) {
            //小于等于三张
            [_photoView setHeight:pgotoW];
            
        }else if (picList.count > 3 && picList.count<=6){
            //小于等于6张大于三张
            [_photoView setHeight:pgotoW + pgotoW + 4];
        }else{
            //大于6张，小于等于9张
            [_photoView setHeight:pgotoW + pgotoW + pgotoW + 4 + 4];
        }
