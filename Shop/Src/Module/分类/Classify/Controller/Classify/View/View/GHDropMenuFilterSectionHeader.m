...
- (void)setDropMenuModel:(GHDropMenuModel *)dropMenuModel {
    ...
    self.details.text =dropMenuModel.sectionSeleted?@"收起":@"展开";
    ...
    CGSize titleSize = [self.title.text sizeWithFont:DR_FONT(16) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
...
- (void)layoutSubviews {
    ...
    CGSize titleSize = [self.title.text sizeWithFont:DR_FONT(16) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
    CGSize detailsSize = [self.details.text sizeWithFont:DR_FONT(14) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
...
- (UILabel *)details {
    if (_details == nil) {
        ...
        _details.textColor = RGBHex(0X888888);
        _details.text = @"展开";
    }
    return _details;
}
