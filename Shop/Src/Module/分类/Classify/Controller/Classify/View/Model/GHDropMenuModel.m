...
- (NSArray *)creaFilterDropMenuData:(NSArray *)bigArr {
    ...
    for (NSInteger index = 0; index < addSourceArr.count; index++) {
        GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
        dropMenuModel.sectionHeaderTitle = addSourceArr[index][@"headTitle"];
        if ([dropMenuModel.sectionHeaderTitle isEqualToString:@"标准"]||[dropMenuModel.sectionHeaderTitle isEqualToString:@"材质"]||[dropMenuModel.sectionHeaderTitle isEqualToString:@"级别"]) {
            dropMenuModel.isMultiple = NO;
        }else
        {
            dropMenuModel.isMultiple = YES;
        ...
        NSArray *contentArr = addSourceArr[index][@"content"];
        NSMutableArray *contentMuArr =[NSMutableArray array];
        if ([dropMenuModel.sectionHeaderTitle isEqualToString:@"标准"]) {
            for (NSInteger index = 0 ; index < contentArr.count; index++) {
                GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
                dropMenuModel.tagName = [contentArr by_ObjectAtIndex:index][@"code"];
                dropMenuModel.tagID = [contentArr by_ObjectAtIndex:index][@"id"];
                dropMenuModel.tagCode =[contentArr by_ObjectAtIndex:index][@"code"];
                dropMenuModel.tagSeleted = NO;
                [contentMuArr addObject:dropMenuModel];
            }
        }else
        {
            for (NSInteger index = 0 ; index < contentArr.count; index++) {
                GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
                dropMenuModel.tagName = [contentArr by_ObjectAtIndex:index][@"name"];
                dropMenuModel.tagID = [contentArr by_ObjectAtIndex:index][@"id"];
                dropMenuModel.tagCode =[contentArr by_ObjectAtIndex:index][@"code"];
                dropMenuModel.tagSeleted = NO;
                [contentMuArr addObject:dropMenuModel];
        ...
        dropMenuModel.dataArray =contentMuArr.copy;
        dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTag;
        [sections addObject:dropMenuModel];
    }
    NSMutableArray *titlesArray = [NSMutableArray array];
    ...
    for (NSInteger index = 0 ; index < 1; index++) {
        ...
        NSNumber *typeNum = types[index];
        dropMenuModel.dropMenuType = typeNum.integerValue;
        dropMenuModel.sectionCount = 2.01f;
        dropMenuModel.menuWidth = [UIScreen mainScreen].bounds.size.width * 0.9;

//        dropMenuModel.dataArray = addSourceArr[index][@"content"];
        dropMenuModel.sections = sections;
