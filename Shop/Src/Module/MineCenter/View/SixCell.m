...
-(void)setHaveShopModel:(GoodsModel *)haveShopModel
{
    ...
    if (haveShopModel.unitConversion1!=0) {
        nameStr =[NSString stringWithFormat:@"%.3f%@/%@",haveShopModel.unitConversion1,baseStr,haveShopModel.unitName1];
    ...
    if (haveShopModel.unitConversion3!=0) {
        nameStr =[NSString stringWithFormat:@"%@ %.3f%@/%@",nameStr?:@"",haveShopModel.unitConversion3,baseStr,haveShopModel.unitName3];
        if (cellStr.length==0) {
    ...
    if (haveShopModel.unitConversion4!=0) {
        nameStr =[NSString stringWithFormat:@"%@ %.3f%@/%@",nameStr?:@"",haveShopModel.unitConversion4,baseStr,haveShopModel.unitName4];
        if (cellStr.length==0) {
    ...
    if (haveShopModel.unitConversion5!=0) {
        nameStr =[NSString stringWithFormat:@"%@ %.3f%@/%@",nameStr?:@"",haveShopModel.unitConversion5,baseStr,haveShopModel.unitName5];
        if (cellStr.length==0) {
...
-(void)setGoodSellOutModel:(GoodsList *)goodSellOutModel
{
    ...
    NSString *baseStr;//basicUnitId 5千支  6公斤  7吨
    baseStr =goodSellOutModel.basicUnitName;
    NSString *nameStr,*cellStr;
    ...
    if (goodSellOutModel.unitConversion2!=0) {
        nameStr =[NSString stringWithFormat:@"%@ %.3ld%@/%@",nameStr?:@"",(long)goodSellOutModel.unitConversion2,baseStr,goodSellOutModel.unitName2];
        if (cellStr.length==0) {
            cellStr =goodSellOutModel.unitName2;
        }
    }
    if (goodSellOutModel.unitConversion3!=0) {
        nameStr =[NSString stringWithFormat:@"%@ %.3ld%@/%@",nameStr?:@"",(long)goodSellOutModel.unitConversion3,baseStr,goodSellOutModel.unitName3];
        if (cellStr.length==0) {
    ...
    self.parameterLabel.text = [NSString stringWithFormat:@"价格：￥%f  包装参数：%@",goodSellOutModel.returnPrice,nameStr];
...
-(UILabel *)cellLabel
{
    if (!_cellLabel) {
        ...
        [_cellLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.standardView.mas_bottom).offset(WScale(11));
            make.left.mas_equalTo(self.productType);
            make.right.mas_equalTo(WScale(-10));
...
-(UILabel *)moneyLabel
{
    if (!_moneyLabel) {
        ...
        [_moneyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.countLabel.mas_bottom).mas_equalTo(WScale(8));
