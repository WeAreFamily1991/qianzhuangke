...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        UIButton * btn = [[UIButton alloc] initWithFrame:CGRectMake(i * witdthFloat, 0, witdthFloat, self.bounds.size.height - 1)];
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]init];
           
            buttonDownView.frame=CGRectMake((3*i+1) * ScreenW/6, self.bounds.size.height - 1, ScreenW/6, 1);
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [buttonDownView setFrame:CGRectMake((3*segument+1)* ScreenW/6,self.bounds.size.height - 1, ScreenW/6, 1)];
        }];
